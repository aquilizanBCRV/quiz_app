...
public class QuizApp_Login {
    ...
    private void createUI(boolean show) {
        ...
        userField.setText("Vince2026");
        passField.setText("rendervoid2026");
...
//                    Globals.getInstance().setUser(checkLogin.getInt("userID"));
    ...
    private static void getUserInfo(ResultSet info) throws SQLException {
        if (info.getString("roles").equals("Student")) {
            login.setRoles("Student");
            login.setUserID(info.getInt("userID"));
            ResultSet userInfo = login.getInfo();
            
            if (userInfo != null && userInfo.next()) {
                Globals.getInstance().setStudentID(userInfo.getInt("studentID"));
                Globals.getInstance().setFirstname(userInfo.getString("firstname"));
                Globals.getInstance().setRoles("Student");
            }
            
        } else if (info.getString("roles").equals("Teacher")) {
            login.setRoles("Teacher");
            login.setUserID(info.getInt("userID"));
            ResultSet userInfo = login.getInfo();
            
            if (userInfo != null && userInfo.next()) {
                
                Globals.getInstance().setTeacherID(userInfo.getInt("teacherID"));
                Globals.getInstance().setFirstname(userInfo.getString("firstname"));
                
                Globals.getInstance().setRoles("Teacher");
            }
